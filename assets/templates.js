(function (global) {
  ...
  function renderOperationBundle(bundle, index) {
    ...
    return [
      ...
      '<tr><th>C_base</th><td>' + fmtMoney(metrics.C_base) + '</td><th>售价 P</th><td>' + fmtMoney(metrics.P) + '</td><th>服务费率</th><td>' + fmtPct(metrics.r_fee) + '</td><th>Fee</th><td>' + fmtMoney(metrics.Fee) + '</td></tr>',
      '<tr><th>佣金率</th><td>' + fmtPct(metrics.r_comm) + '</td><th>佣金额</th><td>' + fmtMoney(metrics.Commission) + '</td><th>不亏损佣金率上限</th><td>' + fmtPct(metrics.maxSafeCommissionRate) + '</td><th>C_actual</th><td>' + fmtMoney(metrics.C_actual) + '</td></tr>',
      '<tr><th>Profit</th><td>' + fmtMoney(metrics.Profit) + '</td><th>ProfitRate</th><td>' + fmtPct(metrics.ProfitRate) + '</td><th>最大可卖套数</th><td>' + fmtInt(stock.maxSets) + '</td><th>可撑GMV</th><td>' + fmtMoney(stock.gmv) + '</td></tr>',
      ...
      '<thead><tr><th>图片</th><th>product_id</th><th>产品名</th><th>成本</th><th>数量</th><th>小计</th><th>库存</th><th>单品可卖套数</th></tr></thead>',
      '<tbody>' + rows + '</tbody>',
      '</table></div>',
      '</section>'
    ].join('');
